...
from nets.transformer import TransformerEncoder
from nets.model_config import get_config
...
class MobileViT(nn.Module):
    ...
    """
    def __init__(self, model_cfg: Dict, embedding_size: int = 512):
        super().__init__()
        ...
        self.classifier.add_module(name="fc", module=nn.Linear(in_features=exp_channels, out_features=embedding_size))
...
def mobile_vit_xx_small(embedding_size: int = 512):
    ...
    m = MobileViT(config, embedding_size=embedding_size)
    return m


def mobile_vit_x_small(num_classes: int = 512):
    # pretrain weight link
...
def mobile_vit_small(num_classes: int = 512):
    # pretrain weight link
